...
# dpa tags some stories with several rubric_names. The python code below can
# boil that down to one
...
# The rubric_name segments below are used to signal "this is currently a top
# story" for 3-12 hours they convey no or little information about the general
# topic and are removed before choosing the single rubric_name.
#
# Sometimes, zero `rubric_names` remain after removing topnews signal
# rubric_names. In this case, the topnews `rubric_name`s are sorted in the order
# listed below and and the first one is chosen

topnews_preference = {
    f'{key}': value for value, key in enumerate((
        '.sportsline.topnews',
        '.topnews',
        '.topthemen',
        '.top-news'))
...
# sometimes, stories fit into two general news `rubric_names`. If so, the first
# one on the list is chosen

infoline_preference = {
    f'weblines.infoline_rs.{key}': value for value, key in enumerate((
        'panorama',
        'wirtschaft',
        'politik.inland',
        'politik.ausland',
...
# sometimes, stories fit into two starline `rubric_names`. If so, the first one
# on the list is chosen
starline_preference = {
    f'weblines.starline.{key}': value for value, key in enumerate((
        'fernsehen',
        'kino',
        'kunst',
        'leute',
...
# sometimes, stories fit into two sportsline `rubric_names`. If so, the first
# one on the list is chosen
sportsline_preference = {
    f'weblines.sportsline.{key}': value for value, key in enumerate((
        'basketball',
        'handball',
        'boxen',
        'e-sport',
...
# sportsline or serviceline items carry many rubric_names by default - see list
# below. These are narrowed down to the top 4 or top 3 hierarchy, if it is
# unique, in the case below "weblines.sportsline.fussball"
#
...
#
# weblines.infoline_rs and weblines.starline overlap in many instances for
# example an item might be assigned `rubric_name`s
# weblines.infoline_rs.boulevard.kultur and weblines.starline.kino
# in these cases, the starline `rubric_name` is dropped if drop_starline is
# True
...
def boildown_starline(names: List[str]) -> str:
    ...
    if len(starline_names) == 1:
        ...
        starlines = [
            {"rubric": rubric, "key": starline_preference[rubric]}
...
def boildown_infoline(names: List[str]) -> str:
    ...
    if any(('infoline_rs' in name for name in names)):
        return sorted(
            names,
            key=lambda name: infoline_preference.get(name, 100))[0]
    return UNDECIDED


def boildown_sportsline(names: List[str]) -> str:
    """
    more than one sportsline ? try to consolidate on top 3 or 4, or else select
    preferred
    """
    if any(('sportsline' in name for name in names)):
        ...
        if single != UNDECIDED:
            ...
            sportslines = [
                {
                    "rubric": rubric,
                    "key": "%03d%s" % (sportsline_preference[topn(rubric, 3)],
                                       rubric)
                } for rubric in names if "sportsline" in rubric]
...
def rubric_name(names: List[str], drop_starline: bool = True) -> str:
    ...
    non_topnews = list((
        name for name in names if not any((name.endswith(signal) for signal in topnews_preference.keys()))
    ))
    ...
    if len(non_topnews) == 0:
        ...
        sorted_topnews = sorted(
            names,
            key=lambda rubric: min((
                pref if rubric.endswith(name) else 9999
